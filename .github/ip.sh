...
curl -o ./china_ipv6_list.txt "https://ispip.clang.cn/all_cn_ipv6.txt"
...
function cnip(){
	...
	result=$(md5sum ./china_ipv6_list.txt)
	md5_1=$(echo $result | awk 'NR==1' | awk '{print $1}')
	md5_2=$(echo $result | awk 'NR==2' | awk '{print $1}')
	if [[ "$md5_1" != "$md5_2" ]];then
		cp -f ./china_ipv6_list.txt ./wireguard/ipv6_network_primary_sample
		cp -f ./china_ipv6_list.txt ./openvpn/ipv6_network_primary_sample
	fi
}
...
function generate(){
	...
	rm ./china_ip_list.txt ./china_ipv6_list.txt ./add.txt ./del.txt
}
